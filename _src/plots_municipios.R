...
plot.nowcast <-
    ...
    geom_point(data = window(now.pred.zoo, end = min(time(now.pred.zoo.original))),
               aes(y=n.casos, col="Notificado"), size=2) +
    geom_point(data= window(now.pred.zoo, start=min(time(now.pred.zoo.original))+1),
...
plot.nowcast.srag <-
    ...
    ggplot(aes(Index)) +
    geom_line(aes(y=estimate.merged), lty=2, col="grey") +
    geom_point(data = window(now.srag.pred.zoo, end = min(time(now.srag.pred.zoo.original))),
               aes(y=n.casos, col="Notificado"), size=2) +
    geom_point(data= window(now.srag.pred.zoo, start=min(time(now.srag.pred.zoo.original))+1),
               aes(y = estimate, col = "Nowcasting"), size=2) +
