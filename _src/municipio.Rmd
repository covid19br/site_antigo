...
### Quantos casos notificados já temos na cidade de `r sigla.municipio[mun]`? Quantos teremos nos próximos dias?

```{r}
ggplotly(municip.plot.forecast.exp[[mun]], tooltip = "text")
```
...
`r  as.integer(municip.exp.5d[[mun]][max(nrow(municip.exp.5d[[mun]])),2])` e 
`r as.integer(municip.exp.5d[[mun]][max(nrow(municip.exp.5d[[mun]])),3])` casos em
`r format(max(time(municip.exp.5d[[st]])), "%d/%m/%Y")` )**, se a taxa de
...
```{r}
ggplotly(municip.tempo.dupl[[mun]])
```   
...
`r round(municip.res.uncertain.si[[mun]]$R[nrow(res.uncertain.si$R), 5],1)` e
 `r round(municip.res.uncertain.si[[mun]]$R[nrow(res.uncertain.si$R), 11],1)` **.
