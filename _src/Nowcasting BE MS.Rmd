...
da ocorrência do óbito. É importante deixar claro que não há somente este modo de informar óbitos, o Ministério
mesmo já informou em alguns de seus boletins óbitos via a data de notificação do óbito. 
Todo o objetivo do _nowcasting_ é realizar uma correção para se chegar a um número mais próximo da realidade, 
sem que se tenha que esperar para isso a chegada de dados no sistema.
O _nowcasting_ corrige esses atrasos que são inerentes ao sistema de notificação.
...
Com cada boletim publicado, o pico (número máximo) de óbitos se desloca para a direita -mais perto do presente-. 
Porém, os óbitos não constam instantaneamente no sistema de notificação, e portanto no final da série há sempre uma queda abrupta. Esta queda é corrigida nos dias seguintes, com a publicação de novos boletins.
O efeito do atraso na notificação também pode ser observado no aumento do pico -que indica que nem todos os óbitos tinham sido confirmados e notificados-. Com cada novo boletim também há um aumento do número de óbitos que aconteceram vários dias atrás. Observe no gráfico como os bolteins mais recentes (em roxo) ainda contribuem bastante para a quantificação de óbitos que aconteceram no passado (mais à esquerda). 
A linha pontilhada vermelha marca o dia de anúncio do primeiro óbito, 
veja que há óbitos ocorridos antes dessa data, mas os boletins mais recentes realizaram essa correção.
...
As colunas mais à esquerda são os boletins mais recentes.
As linhas são as data em que ocorreram óbitos, mais abaixo mais perto do presente.
...
Com isso criamos vetores de datas, teremos que ter dois vetores de tamanho igual à soma da segunda (teste no fim do chunk) coluna da matriz original, boletim mais recente. 
...
Conforme novos óbitos são notificados para um mesmo dia, somente somamos a esse dias esse novo óbito e lhe damos a data de notificação conforme a data do boletim que ele começou a constar.
...
Os specs são o _default_, eles dão os *priors* que serão ajustados aos atrasos e computarão uma distribuição para eles.
O atributo *now* recebe a última data do vetor de datas de ocorrência de eventos.
...
p.prev.ic.cumsum <- ggplot(uol_final4, aes(x = Data, y = `estimate Cumsum`)) +
    ...
    geom_ribbon(aes(ymin = `lower Cumsum`, ymax = `upper Cumsum`),
                fill = "red",
                alpha = 0.15) +
    xlab("Dia do Óbito") +
