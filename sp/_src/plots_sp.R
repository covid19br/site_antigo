...
ncasos.completa <-merge(casos=sampa$casos.acumulados, exp.5d[, c("predito","ic.low","ic.upp")])
...
plot.forecast.exp <-
    ...
    ylab("Número de casos (em SP)") +
    ggtitle("Número de casos notificados em SP em escala logarítimica") +
    plot.formatos
...
plot.tempo.dupl <-
    ...
    ylab("Tempo de duplicação (em SP) (dias)") +
    plot.formatos 
...
plot.estimate.R0 <-
    ...
    ylab("Número de reprodução (em SP)") +
    plot.formatos
